...
## 8. Почта и код подтверждения

### Правило

Код подтверждения — секрет получателя. Он уходит **только письмом** и не
возвращается в ответе API ни при каких настройках. Иначе зарегистрироваться
на чужой адрес смог бы кто угодно: достаточно открыть вкладку «Сеть».

Если письмо не ушло — это отказ, а не повод показать код на экране:

| Ситуация | Ответ API |
|---|---|
| Провайдер не настроен | `503 email_delivery_failed` — «Отправка писем на сервере не настроена…» |
| Провайдер отклонил письмо | `502 email_delivery_failed` — с причиной в логе сервера |
| Письмо ушло | `201` + `resendAfterSeconds`, кода в теле нет |

### Откат неудачной регистрации

Раньше запись пользователя создавалась до отправки письма, поэтому сбой
почты оставлял в базе аккаунт с `email_verified = false`: войти нельзя,
зарегистрироваться заново нельзя («почта занята»). Теперь при неудачной
отправке первого кода свежесозданная запись удаляется — база остаётся
чистой, а человек просто пробует ещё раз.

### Транспорты

| Режим | Условие | Кому доставляет |
|---|---|---|
| `smtp` | `SMTP_HOST` + `SMTP_USER` + `SMTP_PASS` | **любому получателю** — основной режим |
| `resend` | `RESEND_API_KEY` | без подтверждённого домена — только владельцу ключа, поэтому запасной |
| `none` | ничего не задано | никому; регистрация отвечает ошибкой |

Порядок можно переопределить через `MAIL_PRIMARY=resend`.

### Что было сломано и почему

1. **`JWT_SECRET` не был задан.** Этим секретом подписывается сессия *и*
   хешируются коды подтверждения. Без него сервер генерировал случайный
   секрет при каждом запуске, а `npm run dev` (`node --watch`) перезапускается
   на каждое сохранение файла — любой выданный код переставал подходить
   почти сразу. Теперь секрет постоянный, а при его отсутствии сервер
   печатает заметное предупреждение.
2. **Провайдер не был настроен.** `EMAIL_MODE` падал в `dev`, код печатался
   в консоль и возвращался в браузер. Режим `dev` убран целиком.
3. **SMTP мог висеть.** Добавлены `connectionTimeout`, `greetingTimeout`,
   `socketTimeout` и общий предел на отправку — вместо запроса, висящего
   до таймаута браузера, приходит понятная ошибка за секунды.

### Проверка при старте

`verifyMailTransport()` выполняется при запуске сервера: неверный пароль
виден сразу в консоли, а не в момент, когда первый человек нажмёт
«Зарегистрироваться». Состояние доступно в `/health` (`email.ready`,
`email.error`) и в `/api/capabilities` (`email`, `emailError`).

### Аварийный режим для разработки

`ALLOW_DEV_EMAIL_CODES=1` — только если провайдер не настроен: код
печатается **в консоль сервера** и складывается в `lastDevCodes` для
автотестов. В браузер он не попадает никогда. По умолчанию выключено.

### Тесты

```bash
node backend/mail.test.mjs   # 9 проверок политики доставки
node backend/smoke.test.mjs  # 100 проверок API, включая «код не течёт в ответ»
```
